...
has try_built => ( is => ro =>, lazy => 1, builder => sub {
    my ( $self ) = @_;
    return unless $self->has_no_try_built;
    return ! $self->no_try_built;
});
has no_try_built => ( is => ro =>, lazy => 1, predicate => 'has_no_try_built', builder => sub {
    return;
});
has fallback => ( is => ro =>, lazy => 1, builder => sub { 
    my ( $self ) = @_;
    return unless $self->has_no_fallback;
    return ! $self->no_fallback;
});
has no_fallback => ( is => ro =>, lazy => 1, predicate => 'has_no_fallback', builder => sub {
    return;
});
has bootstrap_root => ( is => ro =>, lazy => 1, builder => sub {
    my ( $self ) = @_;
...
has dir => ( is => ro =>, lazy => 1, builder => sub {
    return 'share';
});
